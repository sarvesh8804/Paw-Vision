...
const Hero = () => {
  ...
  // Handle smooth scrolling by 500px per scroll event
  const handleScroll = (event) => {
    event.preventDefault();
    const scrollAmount = event.deltaY > 0 ? 700 : -700; // Scroll down or up
  ...
  const renderFloatingLeaves = () => {
    return Array.from({ length: 5 }).map((_, i) => (
      <div
        key={i}
        ref={(el) => (leafRefs.current[i] = el)}
        className="absolute animate-leaf-sway opacity-50"
        style={{
          top: `${Math.random() * 60 + 10}%`,
          left: `${Math.random() * 80 + 10}%`,
  ...
  return (
    <div 
      ...
      onWheel={handleScroll} // Scroll 500px per event
    >
      {/* Video Background */}
      <video
      ...
      <div className="container mx-auto px-4 relative z-10 h-full flex items-center justify-center">
        <div className="max-w-3xl text-center space-y-6">
          <div
            className="inline-flex items-center px-3 py-1.5 rounded-full bg-white/20 text-white text-sm font-medium mb-2 animate-float-up"
          ...
          <h1
            className="text-4xl md:text-6xl font-bold text-white animate-float-up"
          ...
          <p
            className="text-lg text-white/80 animate-float-up"
          ...
          <div
            className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-4 animate-float-up"
            style={{ animationDelay: "0.8s" }}
          >
            <Link
              to="/predict"
              className="px-6 py-3 rounded-full bg-green-600 text-white font-medium flex items-center gap-2 shadow-lg hover:shadow-xl transition-all hover:translate-y-[-2px]"
            >
              Start Analyzing
              <ArrowRight className="w-4 h-4" />
            </Link>
            ...
            <Link
              to="/about"
              className="px-6 py-3 rounded-full border border-white/40 text-white bg-white/20 backdrop-blur-md font-medium hover:bg-white/40 transition-all"
            >
              Learn More
            </Link>
          </div>
